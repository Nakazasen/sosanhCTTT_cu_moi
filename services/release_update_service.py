...
INSTALLER_NAME = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]*_Setup_[0-9.]+\.exe$", re.IGNORECASE)
...
def _candidate_from_catalog(folder: Path, installed_version: str) -> UpdateCandidate | None:
    ...
    required = {"schema", "version", "installer", "sha256", "size", "notes"}
    if not isinstance(value, dict) or set(value) != required or value["schema"] != 1:
        raise UpdateError("Invalid latest.json schema")
    version = str(value["version"])
    installer_name = value["installer"]
    digest = value["sha256"]
    size = value["size"]
    notes = value["notes"]
    if not INSTALLER_NAME.fullmatch(str(installer_name)) or not isinstance(digest, str) or not re.fullmatch(r"[0-9a-f]{64}", digest.casefold()):
    ...
    if _version(version) <= _version(installed_version):
        return None
    installer = folder / str(installer_name)
    ...
    if not installer.is_file() or installer.stat().st_size != size:
        return None
    return UpdateCandidate(version, installer, size, digest.casefold(), notes)
...
def download_installer(candidate: UpdateCandidate) -> Path:
    ...
    destination = cache / candidate.installer.name
    descriptor, temporary_name = tempfile.mkstemp(prefix="download-", suffix=".tmp", dir=cache)
    os.close(descriptor)
    temporary = Path(temporary_name)
    copied = 0
    try:
        with candidate.installer.open("rb") as source, temporary.open("wb") as target:
